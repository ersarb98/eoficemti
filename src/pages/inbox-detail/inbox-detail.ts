...
export class InboxDetailPage {
  ...
  upload(idSurat, loader) {
    ...
    if (this.fileType == "file") {
      ...
      fileTransfer.upload(this.pathForImage(this.imageFileName), url_upload_sppd, options).then(
        (data) => {
          console.log(data);
          if (data["responseCode"] == 200) {
            var responData = JSON.parse(String(data["response"]));
    ...
    dataUpload.forEach((item, i) => {
      ...
      if ((item["fileName"] != null && item["fileName"] != "") || (item["imageFileName"] != null && item["imageFileName"] != "")) {
        if (item["fileType"] == "file") {
          ...
          fileTransfer.upload(item["fileDocPath"], url_upload_sppd, options).then(
            (data) => {
              if (data["responseCode"] == 200) {
                var responData = JSON.parse(String(data["response"]));
                console.log(responData);
          ...
          fileTransfer.upload(this.pathForImage(item["imageFileName"]), url_upload_sppd, options).then(
            (data) => {
              console.log(data);
              if (data["responseCode"] == 200) {
                var responData = JSON.parse(String(data["response"]));
